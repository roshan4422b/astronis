...
const cities = [
  { name: "Delhi (NCR)", slug: "delhi", region: "New Delhi office · Northern India", lat: 28.61, lon: 77.21, text: "Policy, regulation and the corporate ecosystem.", image: "/images/india-presence/india-gate.jpg", icon: "scale" },
  { name: "Mumbai", slug: "mumbai", region: "Western India", lat: 19.08, lon: 72.88, text: "Financial services, M&A and international business.", image: "/Banking & Financial Services .png", icon: "chart" },
  { name: "Bengaluru", slug: "bengaluru", region: "Southern India", lat: 12.97, lon: 77.59, text: "Technology, startups and innovation.", image: "/Technology, IT & ITES .png", icon: "bulb" },
  { name: "Chennai", slug: "chennai", region: "Southern India", lat: 13.08, lon: 80.27, text: "Manufacturing, automotive and exports.", image: "/Manufacturing & Industrial .png", icon: "building" },
  { name: "Kolkata", slug: "kolkata", region: "Eastern India", lat: 22.57, lon: 88.36, text: "Trade, infrastructure and eastern markets.", image: "/Infrastructure & Projects .png", icon: "globe" },
  { name: "Hyderabad", slug: "hyderabad", region: "Southern & Central India", lat: 17.39, lon: 78.49, text: "Pharma, life sciences and technology.", image: "/Healthcare & Pharmaceuticals .png", icon: "shield" },
  { name: "Ahmedabad", slug: "ahmedabad", region: "Western India", lat: 23.02, lon: 72.57, text: "Manufacturing, textiles and industrial growth.", image: "/Textiles, Apparel & Lifestyle .png", icon: "building" },
  { name: "Chandigarh", slug: "chandigarh", region: "Northern India", lat: 30.73, lon: 76.78, text: "Regional advisory and compliance.", image: "/Professional & Business Services .png", icon: "file" },
  { name: "Lucknow", slug: "lucknow", region: "Northern India", lat: 26.85, lon: 80.95, text: "Government, infrastructure and emerging sectors.", image: "/Real Estate & Construction .png", icon: "people" },
];
const strengths = [
...
function IndiaMap({ hero = false }: { hero?: boolean }) {
  return <svg className={styles.map} viewBox="0 0 400 440" aria-label="India map showing nine key regional business centres">
    ...
    {cities.map(city => {
      ...
      return <a key={city.slug} href={`#city-${city.slug}`} aria-label={`Explore ${city.name}`} className={styles.mapPin}>
        <title>{city.name}</title><circle cx={x} cy={y} r="10" fill={hero ? "#fff" : "#0064bb"} opacity=".15" /><path d="M0 0C-2-3-6-7-6-11a6 6 0 0 1 12 0C6-7 2-3 0 0Z" transform={`translate(${x} ${y})`} fill={hero ? "#fff" : "#0064bb"} stroke={hero ? "#095092" : "#fff"} strokeWidth="1" /><circle cx={x} cy={y-11} r="2" fill={hero ? "#0064bb" : "#fff"} />
        <text x={x + (left ? -10 : 10)} y={y - 5} textAnchor={left ? "end" : "start"} fill={hero ? "#fff" : "#12436e"} fontSize="10" fontFamily="Arial, sans-serif" fontWeight="600" paintOrder="stroke" stroke={hero ? "#124b7e" : "#f4faff"} strokeWidth="3" strokeLinejoin="round">{city.name}</text>
      </a>;
    })}
  </svg>;
}
...
export default function IndiaPresence() {
  return <div className={styles.page}>
    ...
    <section className={`container ${styles.regions}`} aria-labelledby="regions-title"><span className={styles.eyebrow}>REGIONAL FOCUS. NATIONAL CAPABILITY.</span><h2 id="regions-title">Key Regional Strengths</h2><div className={styles.regionGrid}>{cities.map(city => <article id={`city-${city.slug}`} key={city.slug} className={styles.regionCard}><div className={styles.regionImage}><Image src={city.image} alt={city.slug === "delhi" ? "India Gate, New Delhi" : ""} fill sizes="(max-width: 600px) 50vw, (max-width: 1000px) 33vw, 20vw" /></div><div className={styles.regionBody}><span className={styles.regionIcon}><Icon name={city.icon} /></span><span className={styles.regionLabel}>{city.region.split(" · ").pop()}</span><h3>{city.name}</h3><p>{city.text}</p><Link href="/contact#enquiry-form" aria-label={`Discuss advisory support in ${city.name}`}>Discuss Your Needs<Icon name="arrow" /></Link></div></article>)}</div></section>
    ...
    <details className={styles.credits}><summary>Image & map credits</summary><p>India Gate photograph: <a href="https://images.unsplash.com/photo-1587474260584-136574528ed5">Unsplash</a>. Map adapted from <a href="https://github.com/AbhinavSwami28/india-official-geojson">India GeoJSON by AbhinavSwami28</a>, <a href="/images/india-presence/MAP-LICENSE.txt">MIT license</a>. Regional cards use representative sector imagery.</p></details>
  </div>;
}
