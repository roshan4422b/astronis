...
export default function OurStory() {
  return <div className={styles.page}>
    ...
    <section className={styles.journey} aria-labelledby="milestones-title"><div className="container"><div className={styles.sectionHeading}><div><Title label="OUR JOURNEY" id="milestones-title">Key Milestones</Title></div><p>An evolving journey. A stronger tomorrow.</p></div><ol className={styles.timeline}>{milestones.map(([year, title, text]) => <li key={year}><span className={styles.timelineDot} /><span className={styles.year}>{year}</span><h3>{title}</h3><p>{text}</p></li>)}</ol></div></section>
