...
export default function ProfessionalDirectory({ people }: { people: Person[] }) {
  ...
  const visible = people.filter(person => {
    ...
    return text.includes(query.trim().toLowerCase()) && (!focus || text.includes(focus));
  });

  return <>
    <div className={styles.searchBar}><label><span>Search by name or expertise</span><span className={styles.searchInput}><Icon name="search" /><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search professionals" /></span></label><label><span>Area of expertise</span><select value={focus} onChange={event => setFocus(event.target.value)}><option value="">All expertise</option><option value="corporate">Corporate advisory</option><option value="regulatory">Regulatory advisory</option><option value="compliance">Compliance</option><option value="litigation">Litigation & disputes</option><option value="business">Business advisory</option></select></label><button type="button" onClick={() => { setQuery(""); setFocus(""); }}>Reset filters</button></div>
    <div className={styles.resultsHeading}><h3>Our professionals</h3><span>{visible.length} {visible.length === 1 ? "profile" : "profiles"}</span></div>
