...
export default function EnquiryForm({ initialProfessional, initialService, industry }: { initialProfessional?: string; initialService?: string; industry?: string }) {
  ...
  return <form className={styles.form} onSubmit={submit}>
    ...
    <label className={styles.messageLabel}>Your Enquiry / Requirement <em>*</em><textarea name="message" required minLength={10} maxLength={1000} rows={5} value={message} onChange={event => setMessage(event.target.value)} placeholder="Briefly describe your requirement, timeline and any specific questions." /><small>{message.length}/1000 characters</small></label>
    ...
    <div className={styles.submitRow}><button type="submit" disabled={busy}>{busy ? "Sending…" : "Submit Enquiry"}<Icon name="arrow" /></button><p>Our team will review your enquiry and connect you with an appropriate professional.</p></div>
