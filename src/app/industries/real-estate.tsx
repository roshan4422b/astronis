  ...
  return <div className={`${styles.page} ${estate.page}`}>
    <section className={`${styles.hero} ${estate.hero}`} aria-labelledby="estate-title">
      ...
      <div className={`container ${styles.heroInner}`}>
        ...
        <div className={styles.heroCopy}><span className={styles.eyebrow}>Industry Focus</span><h1 id="estate-title">Real Estate &<br />Construction</h1><p className={styles.tagline}>Building Possibilities. Backed by Expertise.</p><p className={styles.heroDescription}>We advise developers, investors, lenders, contractors and occupiers across the real estate and construction value chain, providing practical, commercial and regulatory solutions to help them execute projects successfully and sustainably.</p><Link className={styles.button} href="#enquire">Speak to Our Experts<Icon name="arrow" /></Link></div>
        <div className={styles.heroAside}><span>Building possibilities</span><p>Land<br />Development<br />Regulation<br />Investment<br />Sustainable<br />Communities</p></div>
      </div>
    </section>
    ...
    <section id="overview" className={styles.section} aria-labelledby="overview-title"><div className="container"><div className={styles.overviewGrid}><div><Heading eyebrow="Industry Overview" title="Shaping Smart, Sustainable Spaces" id="overview-title" /><p className={styles.lead}>The real estate and construction sector is a key driver of economic growth, urban development and infrastructure creation.</p><p>We support our clients in navigating complex legal, regulatory and commercial challenges across residential, commercial, industrial and mixed-use projects.</p></div><aside className={styles.sectors}><span className={styles.eyebrow}>Who We Work With</span><h3>Across the real estate value chain.</h3><ul>{audiences.map(([icon, title]) => <li key={title}><Icon name={icon} />{title}</li>)}</ul></aside></div><div className={styles.benefits}>{benefits.map(([icon, title, text]) => <article key={title}><Icon name={icon} /><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
