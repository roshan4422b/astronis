...
export default function Manufacturing() {
  return <div className={styles.page}>
    ...
    <section id="overview" className={styles.section} aria-labelledby="overview-title"><div className="container"><div className={styles.overviewGrid}><div><Heading eyebrow="Industry Overview" title="Driving India’s Manufacturing Momentum" id="overview-title" /><p className={styles.lead}>From the factory floor to global markets, every stage of growth brings new decisions.</p><p>We work with manufacturers to address legal, regulatory and commercial challenges with a practical understanding of their operations. Our integrated advisory brings clarity to complex requirements and helps your team plan for sustainable growth.</p></div><aside className={styles.sectors}><span className={styles.eyebrow}>Who We Work With</span><h3>Expertise across the production landscape.</h3><ul>{["Large, medium & MSME manufacturers", "Export-oriented businesses", "Industrial & engineering goods", "Automobile & auto components", "Chemicals, pharmaceuticals & food processing", "Electronics & precision manufacturing"].map(text => <li key={text}><Icon name="building" />{text}</li>)}</ul></aside></div><div className={styles.benefits}>{benefits.map(([icon, title, text]) => <article key={title}><Icon name={icon} /><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
